...
## 2024-05-18 - Extracted nested components
**Learning:** React re-mounts a nested functional component on each parent re-render, discarding its inner state (like scroll pos).
**Action:** Always define components at the module scope and use `React.memo` paired with `useCallback` hook dependencies for large UI trees.

...
## 2024-05-21 - [Array Spread inside Reduce Bottleneck]
**Learning:** Using `[...acc, ...items]` inside a `reduce` function creates an O(N^2) memory allocation bottleneck, as a new array is instantiated on every single iteration.
**Action:** Always mutate the accumulator directly using `acc.push(...items)` or use `.flatMap()` when aggregating arrays in performance-critical paths.

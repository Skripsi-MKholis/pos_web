...
## 2026-05-18 - Nested Functional Component Performance Antipattern
**Learning:** Defining a functional component inside another functional component's render body (e.g., `const CartContent = () => ( ... )` inside `CashierClient`) is a severe performance anti-pattern. Every state change in the parent (e.g., keystrokes in search) causes a completely new component reference to be created, forcing React to fully unmount and remount the DOM tree instead of diffing it. This destroys local state within the child and causes massive UI lag.
**Action:** Always extract child functional components to the module level (outside the parent function) and pass necessary data/handlers via props. Use `React.memo` for the component and `React.useCallback` for event handlers passed as props to further optimize rendering.

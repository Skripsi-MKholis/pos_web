...
export function CashierClient({ 
  ...
  const updateQuantityCb = React.useCallback((id: string, delta: number) => {
  ...
  const removeFromCartCb = React.useCallback((id: string) => {
    setCart(prev => prev.filter(item => item.id !== id))
  }, [])

  const handleCancelTableOrder = React.useCallback(() => {
    ...
    <div className="flex h-full gap-0 overflow-hidden bg-muted/30 relative">
      ...
      <div className="hidden lg:flex w-[380px] flex-col shrink-0">
        <CartContent
          ...
          handleCancelTableOrder={handleCancelTableOrder}

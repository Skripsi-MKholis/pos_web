...
import { MemoizedCartContent } from "./memoized-cart-content"
...
export const VoucherSection = React.memo(function VoucherSection({
  appliedVoucher, 
  onApply, 
  onRemove, 
  discountAmount, 
...
export function CashierClient({ 
  ...
  const onCancelTable = React.useCallback(() => {
    setSelectedTable(null)
  ...
  const memoizedCartProps = React.useMemo(() => ({
    cart,
    selectedTable,
    onCancelTable,
    updateQuantity,
    ...
    onCheckout,
    VoucherSection
  }), [cart, selectedTable, onCancelTable, updateQuantity, removeFromCart, cartTotal, finalTotal, discountAmount, appliedVoucher, handleApplyVoucher, removeVoucher, isValidatingVoucher, showTables, handleSaveOrder, onCheckout])
  ...
  return (
    <div className="flex h-full gap-0 overflow-hidden bg-muted/30 relative">
      ...
      <div className="hidden lg:flex w-[380px] flex-col shrink-0">
        <MemoizedCartContent {...memoizedCartProps} />
      </div>

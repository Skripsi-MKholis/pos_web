...
// What: Extracted CartContent outside of CashierClient and wrapped in React.memo.
// Why: Defining functional components inside a parent's render scope causes complete
// unmounting/remounting on every render, leading to local state loss and UI lag.
// Impact: Prevents expensive re-renders of the entire cart DOM tree on every keystroke
// or cart update. Measurement: React Profiler will show CartContent no longer re-rendering
// unless its props (cart, finalTotal, etc.) actually change.
...
export function CashierClient({ 
  ...
  // ⚡ Bolt Performance Optimization: Memoize callback to prevent re-rendering CartContent child

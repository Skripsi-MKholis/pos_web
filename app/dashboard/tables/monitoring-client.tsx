...
export function TablesMonitoringClient({ 
  ...
  // ⚡ Bolt: Optimize transaction aggregation by combining multiple reduce passes into a single pass
  // inside useMemo, and replace O(N^2) spread array allocation with direct array mutation (.push)
        ...
        {initialTables.map((table) => {
          ...
          if (txs.length > 0) {
            for (const t of txs) {
              ...
              if (t.created_at < oldestTx.created_at) {
                oldestTx = t
              }
            }
          }
